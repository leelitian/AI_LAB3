...
data = pd.read_csv(r'car.data.txt', encoding='utf-8', header=None)
data.columns.tolist()
data.rename(columns={0: 'buying', 1: 'maintainence', 2: 'doors', 3: 'persons', 4: 'lug_boot', 5: 'safety', 6: 'class'},
            inplace=True)
...
for this_gamma in [1, 10, 50, 100]:
    for this_C in [0.1, 1, 15]:
        ...
        r2_test = clf.score(X_test, y_test)
        print(
            'Gamma = {:.2f},C = {:.2f} \r-squared training: {:.2f}, r-squared test: {:.2f}\n'.format(this_gamma, this_C,
                                                                                                     r2_train, r2_test))
